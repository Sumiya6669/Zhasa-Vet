...
export default function AdminProducts() {
  ...
  const saveMutation = useMutation({
    mutationFn: async (data) => {
      if (data.id) return localDB.entities.Product.update(data.id, data);
      return localDB.entities.Product.create(data);
    },
    onSuccess: () => {
  ...
  const handleImport = async (json) => {
    ...
    await localDB.entities.Product.bulkCreate(items);
    queryClient.invalidateQueries({ queryKey: ['admin-products'] });
  ...
  const handleLoadDemo = async () => {
    await localDB.entities.Product.bulkCreate(DEMO_PRODUCTS);
    queryClient.invalidateQueries({ queryKey: ['admin-products'] });
    queryClient.invalidateQueries({ queryKey: ['products'] });
    toast.success(`Загружено ${DEMO_PRODUCTS.length} демонстрационных товаров`);
  };
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4 flex flex-wrap gap-3 items-center">
        ...
        <div className="flex gap-2 ml-auto flex-wrap">
          <Button variant="outline" size="sm" onClick={handleLoadDemo}>
            <Sparkles className="w-4 h-4 mr-1" /> Загрузить демонстрационные товары
          </Button>
