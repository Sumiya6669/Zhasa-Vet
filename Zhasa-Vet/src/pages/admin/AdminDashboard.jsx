...
import { isAdminLoggedIn, adminLogin, adminLogout } from '@/lib/AuthContext';
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
        <div className="bg-white rounded-2xl border border-slate-200 p-8 w-full max-w-sm shadow-sm">
          <div className="flex items-center gap-3 mb-6">
            <img src="https://media.base44.com/images/public/69d53b93d34df65b976e7cba/c57da13c6_image.png" alt="" className="w-10 h-10 rounded-full" />
            <div>
              <p className="font-bold text-slate-800">ZhasaVet Admin</p>
          ...
          <div className="space-y-3">
            <Input
              ...
              onKeyDown={e => {
                if (e.key === 'Enter') {
                  if (adminLogin(pw)) { setAuthed(true); } else { setPwError(true); }
                }
              }}
              className={pwError ? 'border-destructive' : ''}
            />
            {pwError && <p className="text-xs text-destructive">Неверный пароль</p>}
            <button
              onClick={() => { if (adminLogin(pw)) { setAuthed(true); } else { setPwError(true); } }}
          ...
          <p className="text-xs text-slate-400 text-center mt-4">Пароль: admin123</p>
        </div>
      </div>
    );
  }
      ...
      `}>
        ...
        <div className="px-4 py-4 border-t border-slate-200">
          ...
          <button
            ...
            className="mt-2 flex items-center gap-2 text-sm text-slate-400 hover:text-destructive transition-colors w-full"
          >
            Выйти
          </button>
        </div>
